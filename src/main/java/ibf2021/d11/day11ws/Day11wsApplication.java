...
public class Day11wsApplication {
	...
	private static final Logger logger = LoggerFactory.getLogger(Day11wsApplication.class);
	// Default port used by spring boot application
	...
	public static void main(String[] args) {
		...
		if(optsVal == null || optsVal.get(0) == null) {
			...
			//if both condtions above is not met, get from the args of the app
			portNumber = (String) optsVal.get(0);
		}

		// check if the port number is still null or empty
		if(portNumber != null) {
			// override the spring boot app port number using the defaultproperties from the spring boot
